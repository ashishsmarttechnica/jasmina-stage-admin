...
import useEditPageSection from "../../hooks/PageSection/useEditPageSection";
import usePageSectionValidation from "../../hooks/PageSection/usePageSectionValidation";
import PageSectionForm from "../../components/Forms/PageSectionForm";
// import PageSection from "./PageSection";

const EditPageSection = () => {
  const { handleEditPageSection, loading } = useEditPageSection();
  ...
  const { validateField } = usePageSectionValidation(formData, setErrors, true);
  ...
  const handleSubmit = (data) => {
    ...
    handleEditPageSection(perentId, data);
    setErrors({});
  };
  return (
    <DefaultLayout>
      ...
      <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark p-1 table_no_overflow mb-2">
        <header className="flex justify-between items-center px-2">
          ...
          <Button
            variant="contained"
            onClick={() => navigate("/role-management/admin-management")}
          >
            View All Admin
          </Button>
        </header>
