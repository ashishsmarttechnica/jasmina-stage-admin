...
import useUpcomingExpiredPlan from "../../hooks/ExperiedPlan/useUpcomingexpiredPlan";
import ExpiredPlanTable from "./ExpiredPlanTable";

const UpcomingExpiredPlan = () => {
  const themeMode = useSelector((state) => state?.themeMode?.colorMode);
  const adminData = useSelector((state) => state.loginUser);
  const permission = usePermissions(pageIds.UpcomingExpiredPlan);
  ...
  const {
    ...
    handleEdit,
    handleToggleStatus,
  } = useUpcomingExpiredPlan();
  const navigate = useNavigate();
console.log(data,"sdjfsdjkfdsfh");

  return (
    <>
      <Breadcrumb pageName="Upcoming Expired Plan" />
...
export default UpcomingExpiredPlan; 