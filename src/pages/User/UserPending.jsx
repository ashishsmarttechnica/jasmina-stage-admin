...
const UserPending = () => {
  ...
  const permission = usePermissions(pageIds.User.id,pageIds.User.Userpending);
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 30,
      cellRenderer: (params) => {
        return <p>{params.data?.visibility.isPublic ? 'Yes' : 'No'}</p>;
      },
    },
    {
      headerName: 'LGBTQ +',
      ...
      width: 30,
      cellRenderer: (params) => {
        return <p>{params.data?.visibility.onlyLGBTQFriendlyCompanies ? 'Yes' : 'No'}</p>;
      },
    },
  ...
  useEffect(() => {
    dispatch(getAlluser('', is_pending, page, limit));
  }, [page, limit]);
  ...
  useEffect(() => {
    if (search !== '') {
      ...
      const timer = setTimeout(() => {
        dispatch(getAlluser(search, is_pending, page, limit));
      }, 400);
  ...
  return (
    <>
      <DefaultLayout>
        <Breadcrumb pageName="Pending User" />
