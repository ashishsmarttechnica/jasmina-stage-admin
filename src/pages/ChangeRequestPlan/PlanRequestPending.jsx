...
const PlanRequestPending = () => {
  const permission = usePermissions(
    pageIds.PlanRequestManagement.id,
    pageIds.PlanRequestManagement.PlanRequestPending,
  );
  ...
  const status = 1; // 1 for pending requests
  ...
  const handleReject = (item) => {
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed && result.value) {
        ...
        dispatch(approveRejectPlanRequest(payload))
          .then((res) => {
            if (res.success) {
  ...
  return (
    <DefaultLayout>
      <Breadcrumb pageName="Pending Plan Requests" />
...
export default PlanRequestPending; 