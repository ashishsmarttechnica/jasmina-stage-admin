...
const PostPending = () => {
  ...
  const permission = usePermissions(pageIds.Post.id, pageIds.Post.PostPending);
  ...
  useEffect(() => {
    dispatch(getAllPost('', is_pending, page, limit));
  }, [page, limit]);
  ...
  useEffect(() => {
    if (search !== '') {
      ...
      const timer = setTimeout(() => {
        dispatch(getAllPost(search, is_pending, page, limit));
      }, 400);
        ...
        dispatch(updatePost(item._id, data))
          .then((res) => {
            if (res.success) {
              ...
              toast.error('Failed to approve post: ' + res.message);
            }
          })
          .catch((error) => {
  ...
  return (
    <DefaultLayout>
      <Breadcrumb pageName="Pending Post" />
