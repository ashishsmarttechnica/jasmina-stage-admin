...
const usePageSection = () => {
  ...
  const { data, loading } = useSelector((state) => state?.pageReducerDetails);
      ...
      if (result.isConfirmed) {
        ...
        dispatch(deletepage(id)).then((res) => {
          ...
          if (res.success) {
            ...
            dispatch(getAllPages(search, page, limit));
          } else {
  ...
  const handleEdit = (item) => {
    ...
    const handleDisc = async (id) => {
      dispatch(getAllPages("", page, limit, id))
        .then((response) => {
  ...
  useEffect(() => {
    if (search !== "") {
      ...
      const timer = setTimeout(() => {
        dispatch(getAllPages(search.trim(), page, limit));
      }, 400);
