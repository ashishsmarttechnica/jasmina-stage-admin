...
const useCompanyManagement = () => {
  ...
  useEffect(() => {
    dispatch(getPendingCompany(search,page, limit));
  }, [page, limit]);
  ...
  const searchMyData = async () => {
    try {
      ...
      if (search.trim()) {
        // Dispatch the action to get categories based on the search term
        dispatch(getPendingCompany(search.trim(), page, limit));
      }
    } catch (error) {
