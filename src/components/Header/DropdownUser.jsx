...
const DropdownUser = () => {
  ...
  const userDetail = useSelector((state) => state?.loginUser?.user);
    ...
    <div className="relative">
      ...
      >
        <span className="hidden text-right lg:block">
          ...
          <span className="block text-xs">{userDetail?.role?.role}</span>
        </span>

        <span className="h-10 w-10 rounded-full overflow-hidden">
          <img
            src={`${serverurl}/uploads/${userDetail?.img}`}
            alt="profile"
            className="h-full w-full object-cover"
            onError={(e) => {
              e.target.src = "./noImage2.webp";
            }}
          />
        </span>
      </Link>
  ...
  className={`absolute right-0 mt-4 w-64 flex flex-col rounded-md  border-gray-300 bg-white dark:bg-black dark:text-white shadow-md dark:border-gray-700 dark:bg-gray-800 transition-all duration-300 ${
    dropdownOpen ? "opacity-100 scale-100 visible" : "opacity-0 scale-95 invisible"
  }`}
>
  <div className="px-5 py-4">
    {/* <p className="text-lg font-semibold text-gray-900 dark:text-gray-100 border-b border-gray-200 dark:border-gray-700 pb-3">
      {userDetail?.name}
    </p> */}
    <p className="text-lg text-gray-600 dark:text-gray-400 pt-2">{userDetail?.email}</p>
  </div>

  <button
    className="flex items-center gap-3 px-5 py-3 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-all duration-300"
